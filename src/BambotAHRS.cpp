...
// BambotAHRS.c
//
// Adapted from Paul Stoffregen's MahonyAHRS Library by Greg Armstrong, July 2018
...
void Mahony::update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float deltaT = -1)
{
	if (deltaT == -1) {
		deltaT = invSampleFreq;
	}
...
void Mahony::updateIMU(float gx, float gy, float gz, float ax, float ay, float az, float deltaT = -1)
{
	if (deltaT == -1) {
		deltaT = invSampleFreq;
	}
